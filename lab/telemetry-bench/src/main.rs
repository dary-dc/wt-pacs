        ...
        Ok(match sink {
            ...
            Sink::JsonFile | Sink::BinaryFile => {
                ...
                let path = dir.join(format!(
                    ...
                    std::process::id(),
                    if sink == Sink::JsonFile { "ndjson" } else { "rows" }
                ));
...
fn bench_emit(
    ...
    let (drain, handles_for): (thread::JoinHandle<u64>, Vec<Handles>) = match seam {
        ...
        Seam::OwnBatch => {
            ...
            let handles = (0..producers).map(|_| Handles::Batches(tx.clone())).collect();
            drop(tx);
            let d = thread::spawn(move || {
                let mut n = 0u64;
    ...
    for (i, h) in handles_for.into_iter().enumerate() {
        ...
        workers.push(thread::spawn(move || {
            run_producer(seam, h, i as u64 + 1, emits_per_producer, rate, batch, start)
        }));
    }
    start.wait();
    let t0 = Instant::now();
    ...
    *drained.lock().unwrap() = drained_n;
    ...
    Ok(EmitResult {
        ...
        sink_file_bytes,
        cpus: thread::available_parallelism().map(|n| n.get()).unwrap_or(0),
    })
}
        ...
        Shape::Streaming => {
            ...
            let offline = if offline_exact {
                ...
                for (stage, hist) in [(1usize, &h_work), (2, &h_serve), (3, &h_send)] {
                    ...
                    let hd = hist.dist();
                    for (e, h) in [(exact.p50, hd.p50), (exact.p95, hd.p95), (exact.p99, hd.p99)] {
                        if e > 0.0 {
