...
case "$PROFILE" in
  ...
  30)
    ...
    echo "netem on $IFACE: rate=$RATE loss=${LOSS_PCT}% (no added delay)"
    ;;
  50)
    apply_netem 10 "$LOSS_PCT"
    echo "netem on $IFACE: rate=$RATE delay 10ms loss=${LOSS_PCT}%"
    ;;
  90)
    apply_netem 30 "$LOSS_PCT"
    echo "netem on $IFACE: rate=$RATE delay 30ms loss=${LOSS_PCT}%"
    ;;
  180)
    apply_netem 75 "$LOSS_PCT"
    echo "netem on $IFACE: rate=$RATE delay 75ms loss=${LOSS_PCT}%"
    ;;
  20|60|150)
    one_way=$((PROFILE / 2))
