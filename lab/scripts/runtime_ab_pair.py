...
cols = {"p50_us": (6, 1e-3), "p99_us": (8, 1e-3), "asks_per_s": (10, 1), "cpu_us_per_ask": (11, 1e-3), "ctx_per_ask": (15, 1), "rcvbuf_drops": (16, 1)}
...
for name, (ci, k) in cols.items():
    ...
    for b in arms[1:]:
        ...
        pairs = [(float(by[b][l][ci]), float(by[arms[0]][l][ci])) for l in common if len(by[b][l]) > ci]
