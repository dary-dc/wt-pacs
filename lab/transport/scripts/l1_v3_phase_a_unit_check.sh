...
slow="$(ls "$ROOT"/docs/transport/measurements/r2/raw/l1v3/pilot/S_rtt60_loss2_d4_r1.json)"
...
thin="$ROOT/docs/transport/measurements/r2/raw/l1v3/small/S_rtt60_loss0_d4_r1.json"
set +e; line="$(l1_tail_gate "$thin")"; trc=$?; set -e
echo "thin=$line exit=$trc"
[[ $trc -eq 3 ]] && grep -q 'p95_unsupported' <<<"$line"
thick="$ROOT/docs/transport/measurements/r2/raw/l1v3/small/S_rtt60_loss2_d4_r1.json"
...
set +e; l1_null_gate "$ROOT/docs/transport/measurements/r2/l1_s_vs_q_loss_v3.small.tsv" 15; trc=$?; set -e
echo "null_gate_exit=$trc"
[[ $trc -eq 3 ]]
# A wide bar the same cell can clear, so the gate is not simply always-fail.
set +e; l1_null_gate "$ROOT/docs/transport/measurements/r2/l1_s_vs_q_loss_v3.small.tsv" 500 >/dev/null; trc=$?; set -e
echo "null_gate_wide_bar_exit=$trc"
[[ $trc -eq 0 ]]

echo "== directional header will not truncate tracked results (B4) =="
set +e
l1_write_directional_header "$ROOT/docs/transport/measurements/r2/l1_s_vs_q_loss_v3.small.tsv" 2>/dev/null
trc=$?
set -e
echo "overwrite_guard_exit=$trc"
[[ $trc -ne 0 ]]
[[ "$(grep -vc '^#' "$ROOT/docs/transport/measurements/r2/l1_s_vs_q_loss_v3.small.tsv")" -eq 81 ]]
