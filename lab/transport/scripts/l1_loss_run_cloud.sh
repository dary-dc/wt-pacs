#!/usr/bin/env bash
# Lane L1 — S vs Q under loss (docs/lanes/L1-loss-run.md).
# Round-robin the Oracle São Paulo netem with L2 — never shape while L2 is shaping.
set -euo pipefail
...
OUT_TSV="${OUT_TSV:-$ROOT/docs/transport/measurements/r2/l1_s_vs_q_loss.tsv}"
RAW_DIR="${RAW_DIR:-$ROOT/.local/r2/l1/raw}"
LOG="${LOG:-$ROOT/.local/r2/l1/RUN.log}"
...
# frames_250k_live uses frames_250k D_min (same Tf) when FIXTURES includes it.
declare -A DMIN=(
  [frames_32k|60|S]=8
  [frames_32k|60|Q]=8
  [frames_32k|150|S]=12
...
RTTS=(60 150)
REPEATS=3
# Primary fixture per L1 brief; set FIXTURES="frames_32k frames_250k_live" for the optional 250 KB cell.
FIXTURES="${FIXTURES:-frames_32k}"
...
echo "=== L1 S vs Q loss $(date -Iseconds) ==="
echo "SSH_KEY=$SSH_KEY CLOUD_URL=$CLOUD_URL FIXTURES=$FIXTURES"
echo "RIG: round-robin with L2 — holding netem for this script's lifetime"

[[ -f "$SSH_KEY" ]] || {
  echo "STOP: missing SSH_KEY=$SSH_KEY (see docs/cloud-rig-access.md)" >&2
  exit 1
}

[[ -x "$BIN_MAIN" && -x "$BIN_Q" && -x "$HARNESS" ]] || {
  echo "missing binaries — build with lab/transport/scripts/l1_build_bins.sh" >&2
  exit 1
}
...
# Require nearest-rank harness (L2 brief) before shaped runs.
python3 - <<'PY'
from pathlib import Path
src = Path("lab/window-harness/src/metrics.rs").read_text()
if "percentile_nearest_rank" not in src and "nearest-rank" not in src.lower():
    raise SystemExit(
        "STOP: harness wait_stats is not nearest-rank yet "
        "(docs/lanes/L2-ask-policy.md). Do not start shaped runs."
    )
print("harness p95: nearest-rank present")
PY
...
echo "=== L1 complete $(date -Iseconds) ==="
echo "TSV=$OUT_TSV"
echo "RIG RELEASE: L1 done"
