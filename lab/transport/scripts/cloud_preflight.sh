...
# Usage: lab/transport/scripts/cloud_preflight.sh   Exit: 0 = good to go, 1 = a check failed
set -uo pipefail
...
if [[ $fail -eq 0 ]]; then
  if timeout 25 ssh -i "$SSH_KEY" -o BatchMode=yes -o IdentitiesOnly=yes \
    ...
    if timeout 25 ssh -i "$SSH_KEY" -o BatchMode=yes -o IdentitiesOnly=yes \
      ...
      note "Without it you are back to lab/transport/netsim, which cannot answer CPU or throughput."
    fi
  else
...
echo
if [[ $fail -eq 0 ]]; then
  echo "PREFLIGHT PASSED — continue with docs/transport/measurements/r6/oracle-runbook.md"
  exit 0
fi
