...
OUT_TSV="${OUT_TSV:-$ROOT/docs/transport/measurements/r2/l1_s_vs_q_loss_v2.tsv}"
...
case "$(basename "$OUT_TSV")" in
  l1_s_vs_q_loss_v2.tsv|l1_s_vs_q_loss_v2.VOID.tsv)
    echo "REFUSE: $OUT_TSV is VOID (see docs/transport/measurements/r2/L1_V2_ADVERSARIAL_REVIEW.md). Use the v3 runner." >&2
    exit 2
    ;;
esac
...
[[ -x "$BIN_MAIN" && -x "$BIN_Q" && -x "$HARNESS" ]] || {
  echo "missing binaries — bash lab/transport/scripts/l1_build_bins.sh" >&2
  exit 1
}
