...
# How the rig differs from netsim: docs/transport/measurements/r6/step-scale-calibration.md.
...
cell_params() {
  case "$1" in
    ...
    # Calibrate on the rig: docs/transport/measurements/r6/x3l-run-card.md.
    X3L) echo "25 20 1.0 ${SCALE_X3L:?set SCALE_X3L from a rig calibration - see docs/transport/measurements/r6/x3l-run-card.md - do not reuse the netsim scale or the old 16 default}" ;;
...
for RUN in $(seq 1 "$REPEATS"); do
  for CELL in $CELLS; do
    ...
    for SPEC in "${ARM_LIST[@]}"; do
      ...
      python3 "$ROOT/lab/transport/scripts/r6_row.py" "$EXP" "$LABEL" "$CELL" "$((BASE_RTT+DELAY))" "$RATE" \
...
r6_stop_server || true
echo "--- $OUT"
python3 "$ROOT/lab/transport/scripts/r6_show.py" "$OUT"
