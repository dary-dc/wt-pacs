#!/usr/bin/env bash
# R6 stream shape on netsim. Method: docs/transport/lanes/R6-preregistration.md.
...
# Every scale is frozen from E0-R6b: docs/transport/measurements/r6/step-scale-calibration.md.
cell_params() {
  case "$1" in
...
source "$ROOT/lab/transport/scripts/r6_cell_inputs.sh"
...
for RUN in $(seq 1 "$REPEATS"); do
  for CELL in $CELLS; do
    ...
    for SPEC in "${ARM_LIST[@]}"; do
      ...
      python3 "$ROOT/lab/transport/scripts/r6_row.py" "$EXP" "$LABEL" "$CELL" "$((DELAY*2))" "$RATE" \
