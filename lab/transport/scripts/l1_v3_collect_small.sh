...
# shellcheck source=/dev/null
source "$ROOT/lab/transport/scripts/l1_v3_common.sh"

COMPLETE_PLAN="$ROOT/docs/transport/lanes/L1-v3-complete-plan.md"
PHASE_B_DOC="$ROOT/docs/transport/lanes/L1-v3-phase-b-regime-reader.md"
CADENCE_JSON="${CADENCE_JSON:-$ROOT/docs/transport/measurements/r2/l1_v3_cadence.json}"
PATH_TSV="${PATH_TSV:-$ROOT/docs/transport/measurements/r2/l1_s_vs_q_loss_v3.path.tsv}"
OUT_TSV="${OUT_TSV:-$ROOT/docs/transport/measurements/r2/l1_s_vs_q_loss_v3.small.tsv}"
RAW_DIR="${RAW_DIR:-$ROOT/docs/transport/measurements/r2/raw/l1v3/small}"
SUMMARY_MD="${SUMMARY_MD:-$ROOT/docs/transport/measurements/r2/raw/l1v3/small/DIRECTIONAL_SUMMARY.md}"
...
if [[ "${DRY_RUN:-0}" == "1" ]]; then
  ...
  sample="$(ls "$ROOT"/docs/transport/measurements/r2/raw/l1v3/pilot/S_rtt60_loss0_d4_r*.json | head -1)"
...
if [[ "$SKIP_BUILD" != "1" ]]; then
  bash "$ROOT/lab/transport/scripts/l1_build_bins.sh"
fi
...
"${SCP[@]}" "$ROOT/lab/transport/scripts/l1_veth_setup.sh" "$ROOT/lab/transport/scripts/l1_veth_netem.sh" "$REMOTE:$REMOTE_SCRIPTS/"
