...
case "$PHASE" in
  path)
    exec bash "$ROOT/lab/transport/scripts/l1_v3_path_validate.sh" "$@"
    ;;
  pilot)
    exec bash "$ROOT/lab/transport/scripts/l1_v3_pilot_cadence.sh" "$@"
    ;;
  collect)
    # Phase C directional collect; refuses without APPROVE_SMALL_COLLECT=1.
    exec bash "$ROOT/lab/transport/scripts/l1_v3_collect_small.sh" "$@"
    ;;
  *)
