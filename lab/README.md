...
| Driver | What |
| ------ | ---- |
| `bench/ts_session_stub.mjs BUNDLE` | The product TypeScript client in plain Node against a stub `WebTransport` — no browser, no server; the D2 / D3 reproductions |
...
| `scripts/client_profile_groups.mjs HTTP ARM QUERY OUT` | Like `client_profile.mjs`, but groups main-thread self time by source (recorder, product client, WASM glue, harness, browser) and reads `tap_read_cost_us` when `telemetry=1` |
