...
function nodeRig(impl: Implementation): Rig {
  ...
  return {
    ...
    fake: () => ({
      ...
      trickleFrame: async (i, c, n, ms) => FakeTransport.last.trickleFrame(i, c, n, ms),
...
// Both arms or none: the WASM clock is the bug this suite exists for. docs/cloud-queue.md §Blocked.
if (!wasmBuilt()) {
  console.error(
