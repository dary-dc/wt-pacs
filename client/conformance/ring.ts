/**
 * The wire buffer ring, against every session implementation: what the consumer hands back is
...
/** The fake the implementation dials: installed afresh per fill, driven by its latest dial. */
export type RingFake = {
  install(): void;
  last(): { pushOnOneStream(frames: [number, Uint8Array][]): void };
};
...
async function fill(
  ...
  fake.last().pushOnOneStream(sizes.map((n, i) => [i, body(i, n)] as [number, Uint8Array]));
...
export async function runRing(impl: Implementation, fake: RingFake, check: Check): Promise<void> {
  const twelve = Array(12).fill(4096);

  const ring = await fill(impl, fake, 4, twelve, true);
  ...
  fake.last().pushOnOneStream(twelve.map((n, i) => [12 + i, body(12 + i, n)] as [number, Uint8Array]));
  ...
  const sizes = await fill(impl, fake, 1, [4096, 16, 8192], true);
