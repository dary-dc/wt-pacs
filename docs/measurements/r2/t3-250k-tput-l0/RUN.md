...
Probe-only throughput cell, 0 % iid loss. `REPS=0 PROBE_REPS=6`. No latency repeats. `lab/scripts/stream_shape_cells.sh` from `b213b31`.
...
- cores: 2
- MemTotal: 954 MB
- started: `2026-09-15T02:40:41+00:00`

Cell stderr:

```
qdisc netem 8068: root refcnt 2 limit 1000 delay 30ms rate 10Mbit
  probe shared 4.300 4.30 4.30 4.30 4.30 4.30 4.30 frames/s (median, then each)
  probe pool:2 4.100 4.00 4.00 4.10 4.10 4.10 4.30 frames/s (median, then each)
  probe per-frame 4.300 4.30 4.30 4.30 4.30 4.30 4.30 frames/s (median, then each)
frame=250000B step=326ms (1.4x the rate shared sustained)
wrote 0 repeats and 18 probes to /home/ubuntu/wt-pacs-run/out/t3-250k-tput-l0
```

`RATE_MBIT=10 RTT_MS=60 LOSS_PCT=0 LOSS_MODEL=iid REPS=0 PROBE_REPS=6 PROBE_MS=10000 DEPTH=2`
`--read-bps 0`
