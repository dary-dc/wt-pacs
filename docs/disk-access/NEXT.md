...
| 4 | **`max_udp_payload_size` 1472 → 4000 B** | −35 % CPU, +55 % throughput — the largest lever measured anywhere. **Closed for browsers 2026-09-12:** Chromium's packet reader drops any datagram over 1 472 B ([`../transport/why-these-changes.md` §10](../transport/why-these-changes.md#10--latency-and-throughput-on-one-tree-where-they-part-and-what-joins-them)). Native peers on a jumbo-frame LAN only. [`adr.md`](adr.md) §8 |
