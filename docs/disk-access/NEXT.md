...
| 10 | **The probe is capped at `READ_WINDOW`, the frame is not** | `read_path.rs:221`. Against `hybrid_lazyring` the shipped reader is a coin-flip tie wherever the probe covers the frame and +55 to +82 % RESOLVED wherever it does not. Two cells bracket it: `WTPACS_READ_PATH=uring` on the product arm, the only mode with `probe = false`, and the `pool_capped_probe` arm, which is `pool` with its probe capped at `READ_WINDOW` and nothing else changed. If the first loses the penalty and the second gains it, line 221 is the cause. [`EVIDENCE.md`](EVIDENCE.md) §Where the 250 kB penalty lives |
| 10b | **`pool` and `hybrid_lazyring` at `--depths 2`** | `product_ahead`'s −50 % is `peak_in_flight` 1 → 2, and it appears at 16 KiB where there is no window to serialise. Until the other arms run at depth 2, "look-ahead wins" and "depth 2 wins" are the same number. |
