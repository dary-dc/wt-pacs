...
| 6 | **`read_ahead_kb` and study layout on the target** | Miss rate moved 2–15× by that knob. **E2e on this host (2026-09-10):** a guest-evicted whole-frame fill ties warm (+11 % p50, 6/6, under 28.5 %); the same tile reader at depth 4 is **+200 % p50** when asks stride past the 128 KiB window vs walking the file. The remaining store lever is adjacency (rung-major / grouping), not another reader. Layout study: 17.6× on the same reads (`git show read-path-evidence-2026-09-09:docs/disk-layout/`). Structural; blocked on prefix delivery. Do not re-land `FILL_WINDOW` / fill overlap — [PR #27](https://github.com/dary-dc/wt-pacs/pull/27), [PR #28](https://github.com/dary-dc/wt-pacs/pull/28). |
