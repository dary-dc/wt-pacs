...
| 10 | **The probe is capped at `READ_WINDOW`, the frame is not** | `read_path.rs:221`. Against `hybrid_lazyring` the shipped reader is a coin-flip tie wherever the probe covers the frame and +55 to +82 % RESOLVED wherever it does not. One cell settles it: the same 250 kB cold depth-1 A/B a third time with `WTPACS_READ_PATH=uring`, which is the only mode with `probe = false`. [`EVIDENCE.md`](EVIDENCE.md) §Where the 250 kB penalty lives |
