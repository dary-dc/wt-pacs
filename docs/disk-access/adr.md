...
| **Read ahead (`TILE_SLOTS` / `FILL_AHEAD`)** | B | **+73.8 % asks/s** on missing tiles at depth 2, warm a tie; 16 tiles 1.14 → 0.62 ms; tiles default to 4 slots, fill stays one ahead | four tile slots / two fill buffers after a miss | `slots` is a constructor argument; a fill has no ring | **Accepted** |
