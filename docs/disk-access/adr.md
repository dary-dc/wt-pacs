...
| **`SeqReader` — probe, pool on the miss, one frame named; overlap only when nowait is off; sliding `FILL_WINDOW`** | S | prior tip (overlap + every-miss 4 MiB): warm 16 KiB **tie**; no-nowait 16 KiB **−34.9 % p50, 12/12 RESOLVED**; 250 kB cold **−41.3 % p50, 12/12**, miss 28.9 % / 49.2 %. Combo (that overlap + sliding window + first-miss-only): **measured in [`EVIDENCE.md`](EVIDENCE.md) §Fill overlap** — do not quote the prior tip as the combo. Naive nowait overlap retracted (§2) | 6 threads; **0 rings, 0 fds, 0 memlock** — no ~941-session ceiling | two buffers, no slot table; `posix_fadvise` per quarter window | **Accepted** — combo verdict is the evidence section, not this row's prior-tip numbers |
