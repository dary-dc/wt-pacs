...
**Status:** Accepted · current as of **2026-09-10** (fill: pool-miss overlap + nowait
WILLNEED; naive nowait overlap retracted) · supersedes the 2026-08-31 always-touch
...
| **Scale** | This decision moves about a fifth of a frame's server CPU; per-datagram QUIC work is the rest (§8). Tiles serve at **`TILE_SLOTS` = 4**; fill names one frame ahead, overlaps that read only when nowait is off, and never builds a ring. The loop is a planner over a channel of `Ask`. Depth 2 → 4 was +37 % on the sandbox host |
...
* **A fill never builds a ring.** `SeqReader` has two buffers and the pool. A no-nowait
  ...
  issues WILLNEED and does not overlap pooled reads (`peak_in_flight` 1). A third read
  is never outstanding. `a_fill_starts_the_named_read_before_the_current_miss_is_awaited`,
