...
| **R6** | **ext4 only.** Where `RWF_NOWAIT` is refused (overlayfs, tmpfs, some network filesystems), `pool` and `hybrid` both degrade to `pooled_pread` | Changes which arm is even available, not which is better | **Closed operationally.** `check-fastpath` answers it per host from the server's own probe, and [`DEPLOYMENT.md`](DEPLOYMENT.md) carries the container fix. Measured: ext4 and btrfs honour the flag, overlayfs and tmpfs refuse it. [`IMPLEMENTATION.md`](IMPLEMENTATION.md) makes the refusal case gate the ring off entirely — routing a hit through a ring costs **+131 to +142%**, so the fallback must be `pooled_pread`, never `uring` |
