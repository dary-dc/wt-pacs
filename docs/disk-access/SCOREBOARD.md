...
| **R8** | **`pool` and `hybrid` do not share a code path on a cache hit.** `pool` → `reader_pool` (`depth` Tokio tasks on one atomic), `hybrid`/`uring` → `reader_ring` (one task, `depth` slots). On a hit the ring is never engaged, so the `hit` regime measures reader-loop shape alone | If the loop is a real share of the margin, a restructured pool reader captures it **without** a ring per session | **Answered, and it is core-dependent.** `pool_ringloop` splits the margin (`lab/scripts/s5_split.py`). **4 vCPU** ([`v24`](v24_s5_loop_vs_ring.tsv)): loop a tie everywhere, ring RESOLVED −57% mix / −71% miss. **8 CPU** ([`v25`](v25_s5_laptop-btrfs.tsv)): the **loop RESOLVES on hits at −33.6/−32.9%** (sign 99/113, 102/114) and grows with depth — −0.6% at depth 1 to −45.7% at depth 32, the signature of task contention scaling with parallelism — while the ring stays RESOLVED on misses (−43.1/−42.2%). So **both** terms are real: the loop on hits, and only on hosts with cores to contend for; the ring on misses, everywhere. The idle ring costs +6 to +18% on hits, which is what `hybrid_lazyring` removes ([`v27`](v27_lazyring.tsv)) |
