...
| :---: | --- | --- | --- |
| **R1** | **Single host.** 4-vCPU KVM guest, virtio-blk, ext4, near-idle. One `spawn_blocking` round trip costs a median **34 µs** of process CPU here (p10 26, p90 57 over 252 warm `pooled_pread`-vs-`pool` pairs; 59 µs at one read in flight, ~31 µs from four up) | **This constant is what generates the ring's win.** If the hop is cheap on the production instance, claims 1 and 5 shrink or vanish | **Materially reduced, not closed** — a second host now exists ([`v22_campaign_ci.tsv`](v22_campaign_ci.tsv), AMD EPYC 9V74 / Azure kernel 6.17 / **read-ahead 128 KiB**, i.e. a different CPU vendor *and* the stock window R2b is about). Hop tax **23 648 ns, 0.69× the lab's** — the constant transfers. Ranking: **13 HOLDS · 6 WEAKENS · 5 tie · 0 FLIPS**, and all eight `miss` rows hold at −44.6% to −76.3% with perfect sign agreement. The six WEAKENS are five `mix` rows plus one `hit`, which is where a 64× smaller read-ahead window would bite (R2b). Still open in that this is one extra host, shared and noisy; [`RUN-ON-YOUR-HOST.md`](RUN-ON-YOUR-HOST.md) runs it on a quiet one |
