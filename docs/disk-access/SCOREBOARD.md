...
| :---: | --- | --- | --- |
| **R1** | **Single host.** 4-vCPU KVM guest, virtio-blk, ext4, near-idle. `pooled_pread` prices one `spawn_blocking` round trip at 68–91 µs of process CPU here | **This constant is what generates the ring's win.** If the hop is cheap on the production instance, claims 1 and 5 shrink or vanish | **Open. Highest-value unknown.** Cloud rig and deploy tooling already exist (`lab/scripts/deploy_exact_server_cloud.sh`) |
