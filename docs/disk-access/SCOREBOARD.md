...
| **R5** | **Ring count at scale.** One ring per reader, max 16 readers | Enough concurrent rings would force `io-wq` punting and could reintroduce threads — the exact property claim 4 rests on | Open |
...
| **R8** | **`pool` and `hybrid` do not share a code path on a cache hit**, though the ADR says they do. `read_campaign` reaches a hit through two different loops: `pool` → `reader_pool`, `depth` Tokio tasks contending on one atomic, each with a heap buffer; `hybrid`/`uring` → `reader_ring`, **one** task holding `depth` ring slots with registered buffers. On a hit the ring is never engaged, so `hybrid` vs `pool` there measures **reader-loop shape and nothing else** | Part of the hybrid's margin is attributable to the loop, not to io_uring — and a restructured pool-based reader might capture it **without a ring, an eventfd and registered buffers per session**, which is the cost the ADR rejected the hybrid over. Bounded: in the hit regime the loop is worth **+21% to −35%** depending on host and depth, ~0% at depth 1 on every host (both loops degenerate to sequential — the signature of a concurrency artefact, not a read-path effect) | **Open, and it is now the live question.** Sized by `lab/scripts/loop_shape_control.py` over any campaign TSV. It does **not** overturn the miss-regime result: at depth 64 the loop shape favours `pool` (+12.3% lab, +20.7% CI) and the hybrid still wins the miss regime by **−79.2%** and **−73.3%**, which isolates the ring cleanly. **S5** separates the two |
...
| **S5. A control arm that separates the reader loop from the ring** | R8 | **Low.** One new arm in `read_campaign`: `pool_ringloop` — `reader_ring`'s single-task, `depth`-slot, registered-buffer structure with `spawn_blocking` on the miss instead of the ring. Its delta against `pool` **is** the loop-shape effect; `hybrid` minus it is what io_uring is actually worth. The mirror arm (`reader_pool` + ring on the miss) would confirm it from the other side | **Do before committing to a per-session ring.** The ADR rejected the hybrid on its per-session cost, and the decision doc recommends it on a margin that R8 shows is partly loop shape. This is the cheapest measurement that tells you whether the ring is buying the win or the loop is |
