...
| :---: | --- | --- | --- |
| **R1** | **Single host.** 4-vCPU KVM guest, virtio-blk, ext4, near-idle. One `spawn_blocking` round trip costs a median **34 µs** of process CPU here (p10 26, p90 57 over 252 warm `pooled_pread`-vs-`pool` pairs; 59 µs at one read in flight, ~31 µs from four up) | **This constant is what generates the ring's win.** If the hop is cheap on the production instance, claims 1 and 5 shrink or vanish | **Open. Highest-value unknown.** `lab/scripts/run_read_campaign_cloud.sh` runs the campaign on the rig; `lab/scripts/compare_hosts.py` measures the hop tax on both and says per row whether the ranking HOLDS, WEAKENS or FLIPS |
...
| **R3** | **Synthetic fixture.** 84 MB, uniform 16 KB records, constant bytes, fixed stride. Real studies are tens of GB of variable-size HTJ2K codestreams | Miss rate is the x-axis of the whole decision, and every miss rate here comes from a synthetic stride | **Open. See §6** |
| **R4** | **Force-evicted, not pressure-evicted.** Cells `fadvise(DONTNEED)` the file; they never run with a working set larger than RAM | A study larger than RAM misses because of *eviction*, a regime never measured. Steady-state hit rate under pressure is unknown | **Open. See §6** |
