...
| **R8** | **`pool` and `hybrid` do not share a code path on a cache hit**, though the ADR says they do: `pool` → `reader_pool` (`depth` Tokio tasks on one atomic), `hybrid`/`uring` → `reader_ring` (one task, `depth` slots). On a hit the ring is never engaged, so the `hit` regime measures reader-loop shape alone | If the loop were a real share of the margin, a restructured pool reader would capture it **without** a ring, an eventfd and registered buffers per session | **Answered — the loop is not the explanation.** S5 added `pool_ringloop` (ring-shaped loop, `spawn_blocking` on the miss) and split the margin two runs deep ([`v24_s5_loop_vs_ring.tsv`](v24_s5_loop_vs_ring.tsv), [verdict](v24_s5_loop_vs_ring_verdict.txt)). **Loop alone:** −17.1/−13.2% hit, −2.0/−1.3% mix, +8.6/+10.7% miss — **a tie in every regime, both runs**. **Ring alone:** −56.9/−56.0% mix and −70.7/−72.7% miss, sign agreement 39/40, 42/42, 66/66, 66/66 — **RESOLVED**. io_uring earns the margin; the loop offsets it slightly on misses. The hit-regime spread that raised this (up to −35% across hosts) never resolved under the 28.5% rule — it was read as signal before the rule was applied to it, which is the error the rule exists to prevent. The ring's idle cost is real but also a tie: +7.9/+10.0% on a pure-hit cell where **miss_pct is 0.0% in every arm**, i.e. the price of holding a ring no read ever reaches |
...
| **S5. A control arm that separates the reader loop from the ring** | R8 | Done | **Ran.** `pool_ringloop` is in `read_campaign`; two runs on the sandbox host resolve it — the ring earns the margin, the loop is a tie everywhere. Worth repeating on a host with a different core count before it is treated as settled, since loop shape is where core count would show |
