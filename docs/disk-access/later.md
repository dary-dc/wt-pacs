...
| Idea | Note |
| --- | --- |
| `RWF_NOWAIT` on the real deployment filesystem | **Now answerable in one command:** `check-fastpath <study dir>` shares the server's own probe and exits non-zero where the fast path is absent — see [`DEPLOYMENT.md`](DEPLOYMENT.md), which also gives the container fix. What remains is running it on the real host: overlayfs and tmpfs are measured refusals, XFS and NFS are still unmeasured |
| Ask-queue ahead-N prefetch | **Now measured** ([`PREFIX-READS.md`](PREFIX-READS.md) Part 2): a `POSIX_FADV_WILLNEED` for the next ask is worth 4.6–4.9× on strided cold reads and a loss on sweeping ones. `RequestFrames` already declares a window; `RequestFrame` would need a predictor. What remains is the routing decision, which waits on the layout design |
| Concurrent missing readers | The shape this campaign has never built: several clients at different rungs, all missing, at once. Background sessions in the harness are warmed by construction, so every neighbour cell measures one missing reader. It is also the only shape where queue depth per session could exceed 1 — i.e. the only one where io_uring has anything to spend |
