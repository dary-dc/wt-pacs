...
| [`EVIDENCE.md`](EVIDENCE.md) | **Every number the decision rests on**, in one file: candidates, hosts, risks, what was rejected and why. Includes the **2026-09-09 re-measurement against the code that ships** — every arm and every mmap variant, at 16 KiB *and* 250 kB, with p50 · p90 · p99, CPU per ask, the co-tenant `gap max` and bytes copied |
...
| [`HANDOFF.md`](HANDOFF.md) | **Start here if you are picking this up cold** — what ships, what was decided, what was retracted, and the method rules |
...
| [`READ-PATH-DESIGN.md`](READ-PATH-DESIGN.md) | **Design proposal for iteration**: two app modes (fill / on-demand), why fill needs a reader task, `StreamFrames` + `EndStream`, W per use case, and the order to build in — around the seam the review settles |
