...
| 78 | **HOL1** — one stream against a stream per frame against a pool, under loss, in a browser | queue §Rows 78–81 | **done** 2026-09-25 `d184333` — **no: no `pool:k` is worth it in any lossy cell, and independent delivery moves nothing past 6 %.** Chromium, the raw TS client through `link_impair.py` at 20 Mbit / 80 ms, Cubic, 128 KB frames; `shared` · `per-frame` · `pool:2/4/8`, 7 rounds rotated, 0 % / GE burst / 1 % / 3 %; decision rule pushed before the first run (`T3-stream-shape.md` §HOL1). `D_min` swept per arm: 3 for all. Control passed (`per-frame` −2 % / +0 % against `shared`). Everything received in every cell (280/280 frames, 210/210 asks). Fill-gap p95 / ask p95 against `shared`: `per-frame` −6…+4 % / −2…0 %; `pool:2` asks **+23 %** at 1 % and 3 % (0/7 rounds); `pool:4` / `pool:8` fill **+475 / +268 %** at 1 %, **+583 / +271 %** at 3 % (×2.4 / ×1.3 already at 0 %). Two mechanisms: under random loss Cubic's window is the limit (~1.7 Mbit at 1 %), so a lost packet's round trip is a few percent of a 2 s ask; and QUIC ranks streams, not frames, so a pool stream holding frame `n` takes frame `n + k`'s rank and frames land in bunches (gap p50 0 ms). **Built for it:** `--stream-mode pool:k` and per-frame ask-order priority, ported from `claude/per-core-endpoints` — `per-frame` in this tree now ranks its streams by ask order, where it was FIFO; `shared` stays the default. `lab/stream-shape/`, rows in `docs/measurements/hol1/`. `transport/NEXT.md` row 3 corrected in place. **For row 79:** under BBR the controller stops being the limit, so independent delivery may have something to rescue there; this lane ran Cubic only — `lab/stream-shape/run.mjs` takes a cell and a stream mode, and would take `--congestion` in one line |
| 80 | **RP1** — the range taken in the wrapper's pack, not in a JS pass | queue §Rows 78–81 | **done** 2026-09-25 `043c50e` — **built: a third off a colour fill at 4–6×, and a 16-bit ask halved.** `htj2k_decoder.cpp`'s `pack` takes the integer min/max of the clamped sample and `getRange()` returns it; the wrapper already sign-extends, so `decoder.js` takes the decoder's range when it has `getRange` and runs `finish()` otherwise (the package: unchanged). **Bit-exact** on c512, g512, s12, s512 and sat256, 435 frames — pixels against the package and the encoder's input, range against `finish()` on the package's pixels (`parity.mjs` now checks it). Mutants: colour range from one component (87 differ), one line's range (348), the clamp one short — **caught only by sat256**, which no earlier parity run included. Gate: a dispatch clause with the package and a stand-in glue answering `getRange()` (`client/conformance/range-glue.js`); always-pass and never-pass mutants each fail it. `lab/decode-tail/run.mjs`, source build before/after, every thread throttled, 7 rounds interleaved, none lost: colour all-decoded **−27 / −33 / −35 %** at 1× / 4× / 6× (7/7 each), colour ask −8 / −41 / −36 %, 16-bit ask −18 / −58 / −50 %; the 16-bit fill is wire-bound at 4–6× and does not move; the pack's min/max costs +0.5 ms of WASM at 1×. `decode/README.md` §The range in the pack. **For the workstation:** the win reaches a page only once §The build, as delivered is rebuilt from this wrapper (`ARMS=deliver`). **For every row that times decoding:** `lab/decode-bench/vendor/` present now makes the gate run the real-decoder dispatch clauses, which skip without it |
| 79 | **BB2** — a BBR that keeps its loss tolerance without its queue | queue §Rows 78–81 | **done** 2026-09-25 `54fd3f9` — **yes: BBR with its window held to 1.25× its own path estimate keeps BBR's fill under loss, with no overflow and 13–16 ms of queue.** quinn exposes neither knob (`BbrConfig` sets only an initial window; Cubic's β is a constant), so it is built over the public `Controller` trait as `restart.rs` is: `--congestion bbr-bounded --bdp-gain g` (`server/src/transport/bounded.rs`; four unit claims, each mutant caught). A larger Cubic β would need a Cubic of our own — not built. CC1's rig and cells, rule fixed before the run, 7 rounds: fill at 1 % / 3 % / 1 % behind 1 500 packets — Cubic 43.0 / 74.7 / 41.8 s, BBR 3.88 / 3.87 / 3.89, **×1.25 3.86 / 3.92 / 3.89** (7/7 each); overflow at 200 packets BBR 43–45 %, every bound **0 %**; standing queue BBR 50 / **309 ms**, ×1.25 13 / 16, ×1.0 4 / 4 (Cubic's) at 1.4–1.5× BBR's fill, ×1.5 27 / 31. The blink cell is Cubic's: every BBR 8–10 % slower (×1.0 36 %). Not measured: a competing flow's share (the relay's planes have separate queues) and congestive loss, where §1 found Cubic ahead. **Nothing changed**, Cubic stays; the bound is a candidate for the workstation's rig. `transport/transport-conclusions.md` §1, A bounded BBR. `controller_browser_cells.sh` arms now take server flags (`name:controller::--flag=value`) |
| 81 | **QA1** — row 63's withheld ACK, reproduced in quinn-proto's own test harness | queue §Rows 78–81 | **done** 2026-09-25 `ff75b28` — **reproduced on 0.11.18 and on `main`, and fixed in three hunks; nothing posted.** `ack_is_not_held_back_by_a_full_congestion_window`: 100 ms one way, a two-packet server window, a client PING — the ACK reaches the client **325 ms** after it on 0.11.18 and on `main` `a577f35b` against **225 ms** (latency, `max_ack_delay`, latency) for an idle server; with the fix, 225 on both. The fix, as #2787 shaped `CONNECTION_CLOSE`'s: at the congestion gate a space that owes an ACK builds it alone (not ack-eliciting, so neither congestion-controlled nor paced) through `try_populate_acks`, instead of being skipped. quinn-proto's suite green with it: 304 + 3 (0.11.18), 327 + 3 (`main`). `patches/quinn-proto-0.11.18-ack-when-congestion-blocked.patch` (fix + test), **off**: `patch_crate.sh` does not apply it; it applies cleanly beside `probe-every-space`. Learned for the issue: `main` paces its first flight, so with the default window it fills only as its ACKs return — the test needs a small window, and PT1's session-open probe may be rarer on `main` (harness only). Not measured: whether it removes PT1's probe in Chrome. `transport/upstream-quinn-ack.md` updated in place |
| 82 | **DC2** — the docs cleaned to the essential, in one commit | queue §Row 82 | **ready** — take it only when no other row is `claimed` |
