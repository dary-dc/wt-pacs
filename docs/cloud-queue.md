...
| 41 | **O1** — the fill's order; prerender, yes or no | queue §Rows 30–41 | **done** 2026-09-19 — **S21 measured**: a coarse-to-fine fill takes time-to-scrubbable from 5 688 to 1 043 ms (5.5×) and costs the fill nothing, and the permuted order costs the read path 0.3 % under `--force-pool-reads`, twelve interleaved rounds — `lab/scripts/fill_order_cells.sh`, `transport-conclusions.md` §4. **S20 cannot be answered here**: headless Chromium supports Speculation Rules and never starts a prerender, so the probe (`lab/prerender/`) waits on a headful Chrome — `rig-limits.md` §8 |
