...
| [`2026-09-20.md`](2026-09-20.md) | seventh pass: the integrated branch — fourteen lanes merged green, `GATE OK`, every lever smoked once on the merged binary against the number that owns it, the decoder wrapper's included since 2026-09-22 and its heap column re-measured per arm; and the **fourth identification round**, all of whose rows are now measured and corrected in place — the last two the decoder warm-up, which ships off by default, and a truncated or undecodable frame, which **both** clients now refuse |
...
| **Q57** | defect, decode / clients | the decoder wrapper never throws, and the product reuses one decoder: an undecodable frame reached the consumer as **the previous frame's pixels under the new index**, not as 0 pixels. **Fixed 2026-09-22** — both transports refuse a truncated frame on the wire, `decodeFrame` refuses one the header contradicts, a dying session names every frame its fill still owed, and the per-frame mode is decided, not deferred. What is left is smaller and named: the **BYOB** reader, non-default and only compiled by the gate, still drops a truncated frame silently, and a codestream the **server** truncates before framing passes both checks | ~6 lines for the BYOB reader; a per-frame hash on the wire (row 15's K3) for the server-side truncation | [`../cloud-queue.md`](../cloud-queue.md) §Row 57 · [`../CLIENTS.md`](../CLIENTS.md) §A truncated frame is a failure · [`../decode/README.md`](../decode/README.md) §A frame that did not decode |
...
2026-09-08 pass's own namespace. Its defect is fixed on both clients the same day; it stays on this
list only for the two holes named in its row, neither of which is on a path anyone runs.
