...
| [`2026-09-20.md`](2026-09-20.md) | seventh pass: the integrated branch — thirteen lanes merged green, `GATE OK`, every lever smoked once on the merged binary against the number that owns it, the decoder wrapper's included since 2026-09-22; and the **fourth identification round**, all of whose rows are now measured and corrected in place — the last two the decoder warm-up, which ships off by default, and a truncated or undecodable frame, which is now a failure on the TypeScript transport |
...
| **Q57** | defect, decode / clients | the decoder wrapper never throws, and the product reuses one decoder: an undecodable frame reached the consumer as **the previous frame's pixels under the new index**, not as 0 pixels. **Half fixed 2026-09-22** — the TypeScript transport refuses a truncated frame on the wire and `decodeFrame` refuses one the header contradicts; the **WASM** transport, `failAll` and `--stream-mode per-frame` are not | a `pkg/` rebuild for the WASM reader; `failAll` must call the fill's `onError` before it nulls it | [`../cloud-queue.md`](../cloud-queue.md) §Row 57 · [`../CLIENTS.md`](../CLIENTS.md) §A truncated frame is a failure · [`../decode/README.md`](../decode/README.md) §A frame that did not decode |
