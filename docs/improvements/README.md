...
| **P1** | perf, build | `lto = "fat"` + `codegen-units = 1`: server CPU/frame −5–8 %, `send_us` p50 −8–20 %, binary −26 %, rebuild 3 s → 37 s. On this tree a later cell read p50 **+7 % at depth 1**, one session; saturation at depth 4 stayed a CPU win. Weigh P1 on the depth-1 path if the product ships it | workspace `[profile.release]` | [§P1](2026-09-08.md#p1--server-release-profile-lto--fat-codegen-units--1); [`../transport/why-these-changes.md` §10](../transport/why-these-changes.md#10--latency-and-throughput-on-one-tree-where-they-part-and-what-joins-them) |
