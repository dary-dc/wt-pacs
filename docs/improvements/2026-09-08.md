...
candidate with its evidence, nothing below is landed as code. Front door (ranked open queue):
[`README.md`](README.md). The first pass and its landed commits are in
[`2026-09-06.md`](2026-09-06.md); the complete session state is [`ledger.md`](ledger.md)
(§7 is this pass).
...
- Forty-odd documents under `docs/`, most of them superseded plans kept with banners. This
  lab now lives under `docs/improvements/` with a ranked open queue in `README.md`. A
  whole-`docs/` index (live / superseded / lane-owned) is still open.
...
A later pass on this tree ([`../transport/why-these-changes.md` §10](../transport/why-these-changes.md#10--latency-and-throughput-on-one-tree-where-they-part-and-what-joins-them)
entry 5) read p50 **+7 % at depth 1**, one session, 6/6, while depth 4 at saturation stayed a
CPU win. Weigh the profile on the depth-1 cell if that path ships; do not veto it from the
depth-4 numbers.
...
Drivers that live under `lab/improvements/` are not on this tip. Restore first
([`README.md`](README.md#restore-the-lab-drivers)):
...
node lab/improvements/bench/ts_session_stub.mjs client/transport-ts/dist/session.js
# D4
...
lab/improvements/scripts/sendpath_ab_bench.sh out.jsonl base target/release/exact-server lto /tmp/lto/release/exact-server
# P2: sizes, then init time per variant through the static host
lab/improvements/bench/wasm_variants.sh && ln -s "$PWD/.local/wasm-variants" client/transport-wasm/variants
node lab/improvements/bench/wasm_init_time.mjs http://127.0.0.1:8765 /client/transport-wasm/variants/opt-s-lto 7
# P3: one cell, one arm, recorder off / on
node lab/improvements/scripts/client_profile_groups.mjs http://127.0.0.1:8765 ts "telemetry=0&cell=ondemand&stream_mode=shared&d=4&n=2000&frames=80" off.json
node lab/improvements/scripts/client_profile_groups.mjs http://127.0.0.1:8765 ts "telemetry=1&cell=ondemand&stream_mode=shared&d=4&n=2000&frames=80" on.json
```
