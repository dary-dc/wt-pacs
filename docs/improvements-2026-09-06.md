...
are in git history at commits `e274c26` and `00b15bd` and are regenerated by the drivers in
...
Driver: `lab/scripts/refusals_e2e.mjs` (transcripts in history, `e274c26`).
...
Fix: `try_join_next` after each spawn (one line). Rows in history (`e274c26`).
...
(`lab/scripts/callgrind_run.sh`; the annotated outputs are in history, `e274c26`).
...
profiles and the summary are in history, `e274c26`):
