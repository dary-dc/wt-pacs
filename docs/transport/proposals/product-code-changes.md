...
marked below with what actually landed, and none was applied by this document. Remaining
proposals (§2 harness timing, §3 want-stamp, §6 exclusive crypto features, §8 chunked
re-fault) do not move the product needle; §6 is the only one that touches `server/` and
it is a silent-bench guard. Everything still marked *proposed* is a proposal. **No code in `server/` or
...
merely wastes a review. This lane's drivers are under `lab/transport/scripts/`.
