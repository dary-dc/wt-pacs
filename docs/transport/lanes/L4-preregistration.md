...
or D (decision rule).** Raw data: `docs/transport/measurements/l4/`.
...
| ---- | ------------ | ------------ | ---------- | ------------------ |
| `lab/transport/netsim` | userspace UDP delay / loss / rate / finite queue | E0 below | latency, loss, congestion-control questions | CPU, throughput ceilings |
...
outside the tree (`lab/transport/scripts/quinn_lab_build.sh`), which is why they exist only in
`target/lab-arms/`.
