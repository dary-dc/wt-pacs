...
`lab/transport/scripts/{l1_loss_run_v2_cloud.sh,l1_build_bins.sh}`, `lab/scripts/{cloud_netem.sh,cloud_common.sh,gen_tf_fixtures.sh}`,
...
| **7** | **Analysis script first** | `lab/transport/scripts/l1_v3_analyze.py` with S12 + **C2**; committed **before** any row (Gate S12) | ~2 h | 8 |
| **8** | **Protocol freeze** | S11 dirty-tree check, `protocol_sha` column | 30 min | 9 |
| **9** | **Collect** | S6 interleaved, S8 raw to `docs/transport/measurements/r2/raw/l1v3/`, S9 repeats, A6, A4 | — | 10.5 h rig |
...
| `lab/transport/scripts/l1_build_bins.sh` | **C1** build Q from `feat/set-priority-per-frame-v3`; emit both server SHAs |
| `lab/transport/scripts/l1_loss_run_v3_cloud.sh` | **New.** netns exec, three-port arms (C3), interleaving (S6), per-run netem assert (S7), pilots (A1), gates S3/S4a/S4b/S5/A2/A5, raw to a tracked dir (S8), `protocol_sha` (S11) |
| `lab/transport/scripts/l1_v3_analyze.py` | **New.** Medians, bootstrap CI (20 000), permutation p, the S12 + C2 decision test, cell labels |
...
| `docs/transport/measurements/r2/l1_s_vs_q_loss_v2.tsv` | S0 → `…v2.VOID.tsv` with the VOID header line |
...
Raw per-run JSON and the `tc` snapshot go to `docs/transport/measurements/r2/raw/l1v3/` — **tracked**, ≈ 2 MB
