...
This lane's write-up and drivers live under [`docs/transport/`](.) and
[`lab/transport/`](../../lab/transport/). Product ADRs and `main`'s stream-mode / L2
docs stay at `docs/` / `docs/lanes/` / `docs/measurements/`.
...
`lab/transport/scripts/r6_cell_inputs.sh`. `SCALE_X3L` also had a default of 16 — netsim's 32 halved
...
**2026-09-09.** No further testing. The results and the applied product decisions are
enough to ship `shared` + `chunked`. The leftover list below does not move that needle.
Proposal §6 (exclusive crypto features) is the only product-adjacent leftover; it is a
silent-bench guard, not a throughput change. Sampler deploy, §2 / §3 / §8, and a
re-run of the stall campaign are not merge work.
...
The stall campaign on the merged tree is **not a merge gate**. The type-level check is
`frame_bytes_is_a_view_of_the_mapping`; the write paths share assemblers via
`all_send_paths_are_the_same_wire`. Megabytes-per-connection would mean the copy is back,
but that campaign is not being re-run.
...
python3 lab/transport/scripts/classify_loss_regime.py /var/log/wtpacs/path.jsonl --per-session
```
...
- ~~**The pathological client**~~ — **done 2026-09-07.** `window-harness --mode stall`,
  ...
  *widens* its own receive window first is a different threat model, and on this rig it does
  ...
  can bind. `lab/transport/scripts/stall_wide_window_probe.sh` sweeps it. Whether that holds on a
...
| script | purpose |
| ------ | ------- |
| `lab/transport/scripts/cloud_preflight.sh` | **run first** on any rig work; fails fast if the environment cannot reach it |
| `lab/transport/scripts/e0_r6_reader_validate.sh` | proves the rig can produce head-of-line blocking |
| `lab/transport/scripts/e0_r6_calibrate.sh` | finds the admissible operating point; takes `SEED=` |
| `lab/transport/scripts/e0_r6_calibrate_cloud.sh` | the rig variant; takes `REPS=` for E0-R6c and **`SRV_EXTRA=`**, so the operating point is calibrated in the condition the campaign actually runs |
| `lab/transport/scripts/e0_regime_validate.sh` | proves the loss-regime classifier on known ground truth |
| `lab/transport/scripts/r6_campaign.sh` | the stream-shape campaign |
| `lab/transport/scripts/r6_analyse.py` | applies the pre-registered decision rules |
| `lab/transport/scripts/mem_per_connection.sh` | memory per viewer; takes `READ_BPS=` for the stress case |
| `lab/transport/scripts/classify_loss_regime.py` | offline regime classification |
| `lab/transport/scripts/r6_cell_inputs.sh` | refuses a campaign whose cells and fixture/trace disagree — X3L and X3S are *defined* by varying those |
| `lab/transport/scripts/e0_stall_validate.sh` | proves `--mode stall` really stops reading; gates the campaign below |
| `lab/transport/scripts/stall_client_campaign.sh` | the pathological-client campaign; samples **both** ends |
| `lab/transport/scripts/stall_analyse.py` | per-connection slopes, server and client |
| `lab/transport/scripts/stall_send_path_probe.sh` | rules out the chunked-send-path/`RssAnon` confound |
| `lab/transport/scripts/stall_wide_window_probe.sh` | the hostile variant: client widens its own window first |
