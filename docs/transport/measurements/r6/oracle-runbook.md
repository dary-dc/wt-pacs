...
> `e0_r6_calibrate.sh` / `r6_campaign.sh`, which are **localhost + `lab/transport/netsim` only** and
...
```bash
DELAY=25 RATE=20 LOSS=0.1 lab/transport/scripts/e0_r6_reader_validate.sh
```
...
```bash
DELAY=25 RATE=20 LOSS=0.1 SCALES="1 2 4 8" lab/transport/scripts/e0_r6_calibrate.sh
```
...
```bash
EXP=r6cloud CELLS="X1 X2 X3 N0" lab/transport/scripts/r6_campaign.sh 3
