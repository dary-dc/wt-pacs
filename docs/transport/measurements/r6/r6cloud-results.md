...
rather than against `lab/transport/netsim` on localhost.
...
`docs/transport/transport-conclusions.md` §2 currently says: *"Per-frame is 3.5× worse at 1 % loss and
...
`lab/transport/scripts/r6_adversarial_checks.py` runs them.
...
forces batch = 1 and restores exactly the i.i.d. per-datagram loss that `lab/transport/netsim` models,
...
DELAY=25 RATE=20 LOSS=1.0 SCALES="2 4 6 8" REPS=3 lab/transport/scripts/e0_r6_calibrate_cloud.sh
...
EXP=r6gso_on  CELLS="X3 N0" lab/transport/scripts/r6_campaign_cloud.sh 3
