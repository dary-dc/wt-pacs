...
| **Preflight** | `lab/transport/scripts/cloud_preflight.sh` — fails fast if the environment cannot reach the rig. |
...
`lab/transport/scripts/r6_cell_inputs.sh`, which refuses the run. Two consequences worth knowing:
...
FIXTURE=frames_500x250k DELAY=25 RATE=20 LOSS=1.0 \
  SCALES="4 8 16 32" REPS=1 lab/transport/scripts/e0_r6_calibrate_cloud.sh
```
...
```bash
FIXTURE=frames_500x250k DELAY=25 RATE=20 LOSS=1.0 \
  SCALES="<chosen>" REPS=3 lab/transport/scripts/e0_r6_calibrate_cloud.sh
```
...
EXP=r6cloud_x3l CELLS="X3L" FIXTURE=frames_500x250k SCALE_X3L=<calibrated> \
  lab/transport/scripts/r6_campaign_cloud.sh 3

python3 lab/transport/scripts/r6_analyse.py .local/measurements/r6/r6cloud_x3l.tsv
python3 lab/transport/scripts/r6_adversarial_checks.py .local/measurements/r6/r6cloud_x3l.tsv
```
