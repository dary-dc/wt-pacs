...
| ---- | --- | ----------- |
| `loopback_arms.tsv` | `lab/transport/scripts/quic_opt_bench.sh` | send path, crypto provider, GSO, windows, MTU, ACK frequency, congestion, socket buffers, `send_fairness` |
| `shaped_arms.tsv` | `lab/transport/scripts/quic_opt_shaped.sh` | the same arms at 47 and 189 Mbps achieved |
| `copy_knee.tsv` | `lab/transport/scripts/quic_opt_shaped.sh` | copy vs chunked across five rates — the knee sweep `send-path-copy-costs.md` asked for |
| `aead.tsv` | `lab/transport/aead-bench` | ring vs aws-lc-rs, per-packet seal at 1200 / 1452 / 14520 B |
