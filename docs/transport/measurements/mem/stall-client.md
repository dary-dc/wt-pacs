...
Gate: `lab/transport/scripts/e0_stall_validate.sh` — **passed in both stream modes** before any row
below was collected.
Raw data: [`stall_client.tsv`](stall_client.tsv) — **48 rows, 0 VOID**.

```bash
bash lab/transport/scripts/e0_stall_validate.sh                 # must pass first
SM=per-frame SPORT=14653 bash lab/transport/scripts/e0_stall_validate.sh
REPEATS=3 NS="1 4 8 16" ASKS=400 bash lab/transport/scripts/stall_client_campaign.sh
python3 lab/transport/scripts/stall_analyse.py .local/measurements/mem/stall_client.tsv
```
...
`lab/transport/scripts/stall_send_path_probe.sh`, analysed by `lab/transport/scripts/stall_send_path_analyse.py`. **The prediction was right, and the artefact is not
...
- **The client here is stalled, not hostile.** It uses stack-default windows. A client that
  *widens* its own receive window first is a different threat model —
  `lab/transport/scripts/stall_wide_window_probe.sh` sweeps it, and see §5 below.
