...
| Segments per `sendmsg`, PGO and the pooled hand-off cut CPU per byte | **Only with §8 underneath. Re-measured without it 2026-09-18: the CPU saving holds (−16 to −22 %) and throughput falls 29–38 % (6/6, two cells), so the win was the runtime shape's, not the per-byte work's.** Strong on this VM: −24 to −35 % combined, 6/6 in seven of eight pinned cells, two independent runs; one lab cell (250 KB, depth 1, one session) loses 15 % throughput. Not run on the target |
