//! Server-side SBND reader. Why not a memory mapping: `docs/disk-access/adr.md`.
...
/// Bytes per `read_at_nowait` on the serving path. A miss is not bounded by this —
/// see `media::read_path`. Why 64 KiB: `docs/disk-access/adr.md`.
...
/// One open study. Shared by every session reading it — never opened per session
...
impl FrameStore {
    ...
    /// Whether this study's filesystem honours `RWF_NOWAIT`. Overlayfs/tmpfs refuse it,
    /// so every read reports a miss. Callers that branch on a miss must gate on this too.
    pub fn nowait_supported(&self) -> bool {
        self.nowait
    }

    /// The study file. One fd per study, however many sessions read it.
    pub fn file(&self) -> &File {
        &self.file
    }

    /// Bytes to attempt per serving-loop round. The whole frame where `RWF_NOWAIT` is
    /// refused, so that host pays one pooled read per frame rather than one per window.
    ...
    /// `preadv2(RWF_NOWAIT)`: copies what is already in the page cache, never waits.
    /// Safe on the Tokio executor. Short means the rest must be read where blocking is
    /// allowed. `0` if the filesystem refuses the flag — degrade to the pool, do not fail.
    ...
    /// Make every `read_at_nowait` report a miss. Tests use this rather than
    /// `fadvise(DONTNEED)`, which is not a reliable eviction.
    #[cfg(test)]
...
/// `EAGAIN` is support (the flag working on a cold byte). Only an outright refusal
/// means no fast path; anything unexpected is treated the same, conservatively.
...
/// Same probe as [`FrameStore::open`], for `tools/check-fastpath`. A directory gets a
/// temporary file inside it: support is a property of the mount, not of a study file.
pub fn nowait_supported_at(path: &Path) -> Result<bool> {
    ...
    // A hole still reports whether the filesystem implements the flag.
...
mod tests {
    ...
    /// Variable-length frames: two lengths is the smallest case that catches a reader
    /// that assumed fixed size.
    #[test]
    ...
    /// `check-fastpath` and `FrameStore::open` share `probe_nowait`; this pins they agree.
    #[test]
    ...
    /// A short `read_at_nowait` must be completable by `read_at_blocking` at the offset
    /// it stopped at.
    #[test]
